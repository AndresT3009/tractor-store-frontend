...
import { CHECKOUT_API_URL } from './app/checkout-api-url.token';
...
// apunta a localhost:8080, igual que el default de CHECKOUT_API_URL, así que sobreescribirlo aquí
// no cambia nada en dev.
async function loadApiUrlProvider() {
  const config = await fetch('runtime-config.json').then((res) => res.json());
  return { provide: CHECKOUT_API_URL, useValue: config.apiUrl };
}
