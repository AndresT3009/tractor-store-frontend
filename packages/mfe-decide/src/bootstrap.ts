...
import { DECIDE_API_URL } from './app/decide-api-url.token';
...
// apunta a localhost:8080, igual que el default de DECIDE_API_URL, así que sobreescribirlo aquí
// no cambia nada en dev.
async function loadApiUrlProvider() {
  const config = await fetch('runtime-config.json').then((res) => res.json());
  return { provide: DECIDE_API_URL, useValue: config.apiUrl };
}
